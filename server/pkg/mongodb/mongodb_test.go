...
func TestAppendEvent(t *testing.T) {
	...
	t.Run("EventStreamEmpty", func(t *testing.T) {
		...
		for eventCursor.Next(ctx) {
			var event mongodb.Event
			err := eventCursor.Decode(&event)
			require.NoError(t, err)

			assert.Equal(t, streamID, event.StreamID)
			assert.Equal(t, expectedVersion+1, event.StreamVersion)
			assert.Equal(t, eventType, event.Kind)
	...
	t.Run("EventStreamExists", func(t *testing.T) {
		...
		for eventCursor.Next(ctx) {
			...
			if eventCount == 0 {
				...
				assert.Equal(t, int64(1), event.StreamVersion)
			} else if eventCount == 1 {
				assert.Equal(t, eventData2, event.Data)
				assert.Equal(t, int64(2), event.StreamVersion)
			} else {
	...
	t.Run("OptimisticConcurrencyViolation", func(t *testing.T) {
		...
		require.Error(t, err)
		assert.Nil(t, res)
		assert.Contains(t, err.Error(), mongodb.ErrInvalidStreamVersion)
...
func BenchmarkAppendEvent(b *testing.B) {
	...
	for eventsCursor.Next(ctx) {
		...
		assert.Equal(b, eventCount, event.StreamVersion)
		assert.Equal(b, eventType, event.Kind)
